...
# Tōhoku Tsunami Impact on Albatross Populations in the Pacific
...
# head(albie_band)
# unique(albie_band$event_year)
# unique(albie_band$species_name)
# unique(albie_band$event_date)
# unique(albie_band$iso_subdivision)
# class(albie_band$event_date)
# class(albie_band$event_year)
```
...
```{r create HI map}
# bounding box for Hawaii  lowerleftlon, lowerleftlat, upperrightlon, upperrightlat
...
albie_map <- ggmap(hawaii_map) +
  ...
  scale_color_manual(name = "Albatross species",
   ...
   labs(title = "Banded Albatross Species in the Hawaiian Archipelago",
        subtitle = "1996 - 2021",
        x = "Longitude",
        y = "Latitude") +
...
```{r}
ggsave(here("images/albatross_map.png"), plot = albie_map)
```
...
albie_count_plot <- ggplot(band_count, aes(x = year, y = total, group = species)) +
  ...
  annotate("text",
           ...
           color = "black",
           size = 4) +
  labs(title = "Albatross Count Based On Band Data",
...
```{r}
ggsave(here("images/species_count_plot.png"),
...
```{r}
ggsave(here("images/bfal_qqplot.png"),
       width = 4,
       height = 4,
       units = "in",
       bg = "white",
...
```{r}
ggsave(here("images/laal_qqplot.png"),
       width = 4,
       height = 4,
       units = "in",
       bg = "white",
...
$$ \text{Albatross population trend}_i = \beta_0 + \beta_1 \text{post tsunami}_i + \varepsilon_i $$ $$ \text{Albatross count}_i = \beta_0 + \beta_1 \text{count_year-1} + \beta_2 \text{tsunami event}_i + \varepsilon_i $$

$$ \text{Albatross count}_i = \beta_0 + \beta_1 \text{count_year-1} + \beta_2 \text{tsunami event}_i + \varepsilon_i $$ $$ \text{Albatross count}_i = \beta_0 + \beta_1 \text{tsunami event} + \varepsilon_i $$
...
tab_model(post_tsunami_simple_mod ,
          pred.labels = c("Intercept (avg. albatross banded pre-tsunami", "Avg. banded albatross count post-tsunami)"),
          string.ci = "Conf. Int (95%)",
          string.p = "P-value",
          title = "Tbl #. Simple Linear Regression Model Results for Banded Albatross in Hawaii & California",
          digits = 4)
```
