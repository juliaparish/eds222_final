...
$$ \text{Albatross population trend}_i = \beta_0 + \beta_1 \text{post tsunami}_i + \varepsilon_i $$
...
```{r}
bothspecies_df <- bind_rows(bfal_band, laal_band)
```

```{r}
bothspecies_df$t_event <- as.factor(bothspecies_bands$t_event)
```

```{r}
post_tsunami_simple_mod <- lm(total ~ t_event, data = bothspecies_df) %>%
  summary()
```
...
```{r}
# interaction
interaction_model <- lm(total ~ t_event + species + t_event:species, data = bothspecies_bands) %>% 
  summary()
```
